...
# data paths
session_data = 'data/driving_log.csv'
...
def generate_batch_samples(path, batch_size=128, path_prefix='data/'):
    ...
    while 1:
        ...
        for line in f:
            ...
            if cam_view == 'left':
                # left image
                path = path_prefix + data_id[1].strip()
...
def nvidia_model(time_len=1):
    ...
    model.add(Convolution2D(24, 5, 5, subsample=(2, 2),
                            border_mode="valid", init=INIT, W_regularizer=l2(reg_val)))
...
my_samples_gen = generate_batch_samples(udacity_data, batch_size=128)
u_samples_gen = generate_batch_validate(udacity_data, batch_size=200)
